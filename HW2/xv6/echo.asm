...
printint(int fd, int xx, int base, int sgn)
{
 ...
 38d:	0f b6 82 c5 06 00 00 	movzbl 0x6c5(%edx),%eax
...
printf(int fd, char *fmt, ...)
{
 ...
 4c8:	89 4d f0             	mov    %ecx,-0x10(%ebp)
        if(s == 0)
 4cb:	b9 be 06 00 00       	mov    $0x6be,%ecx
...
free(void *ap)
{
 ...
 561:	8b 0d e0 06 00 00    	mov    0x6e0,%ecx
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5ac:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 5ad:	89 0d e0 06 00 00    	mov    %ecx,0x6e0
}
 5b3:	c3                   	ret    
  bp = (Header*) ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
 ...
 5d4:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 5d5:	89 0d e0 06 00 00    	mov    %ecx,0x6e0
}
 5db:	c3                   	ret    
...
malloc(uint nbytes)
{
 ...
 5e9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 5ec:	8b 15 e0 06 00 00    	mov    0x6e0,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
malloc(uint nbytes)
{
 ...
 60c:	8d 3c dd 00 00 00 00 	lea    0x0(,%ebx,8),%edi
    ...
    if(p == freep)
 613:	3b 0d e0 06 00 00    	cmp    0x6e0,%ecx
 ...
 632:	89 59 04             	mov    %ebx,0x4(%ecx)
      ...
      freep = prevp;
 635:	89 15 e0 06 00 00    	mov    %edx,0x6e0
      return (void*) (p + 1);
...
morecore(uint nu)
{
 ...
 668:	e8 f3 fe ff ff       	call   560 <free>
  return freep;
 66d:	8b 15 e0 06 00 00    	mov    0x6e0,%edx
      }
      freep = prevp;
      return (void*) (p + 1);
    }
...
morecore(uint nu)
{
 ...
 68b:	eb a8                	jmp    635 <malloc+0x55>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
 68d:	ba d8 06 00 00       	mov    $0x6d8,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 692:	c7 05 e0 06 00 00 d8 	movl   $0x6d8,0x6e0
 699:	06 00 00 
 69c:	c7 05 d8 06 00 00 d8 	movl   $0x6d8,0x6d8
 6a3:	06 00 00 
    base.s.size = 0;
 6a6:	c7 05 dc 06 00 00 00 	movl   $0x0,0x6dc
 6ad:	00 00 00 
