...
panic(char *s)
{
  ...
  printf(2, "%s\n", s);
     ...
     3fc:	e8 7f 0b 00 00       	call   f80 <printf>
  exit();
     ...
     85f:	89 5c 24 04          	mov    %ebx,0x4(%esp)
{
  ...
  if(s != es){
    ...
    printf(2, "leftovers: %s\n", s);
     ...
     88a:	e8 f1 06 00 00       	call   f80 <printf>
    panic("syntax");
...
getcmd(char *buf, int nbuf)
{
  ...
  printf(2, "$ ");
     ...
     911:	e8 6a 06 00 00       	call   f80 <printf>
  memset(buf, 0, nbuf);
...
runcmd(struct cmd *cmd)
{
  ...
  case EXEC:
    ...
    printf(2, "exec %s failed\n", ecmd->argv[0]);
     ...
     99b:	e8 e0 05 00 00       	call   f80 <printf>
    bcmd = (struct backcmd*)cmd;
    if(fork1() == 0)
      runcmd(bcmd->cmd);
    break;
  ...
  case REDIR:
    ...
    if(open(rcmd->file, rcmd->mode) < 0){
     ...
     b1d:	e8 5e 04 00 00       	call   f80 <printf>
      exit();
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
     ...
     c03:	e8 78 03 00 00       	call   f80 <printf>
...
putc(int fd, char c)
{
     ...
     ed3:	83 ec 18             	sub    $0x18,%esp
     ed6:	88 55 fc             	mov    %dl,-0x4(%ebp)
  write(fd, &c, 1);
     ed9:	8d 55 fc             	lea    -0x4(%ebp),%edx
     edc:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
     ee3:	00 
     ee4:	89 54 24 04          	mov    %edx,0x4(%esp)
     ee8:	89 04 24             	mov    %eax,(%esp)
     eeb:	e8 58 ff ff ff       	call   e48 <write>
}
     ef0:	c9                   	leave  
     ef1:	c3                   	ret    
     ef2:	8d b4 26 00 00 00 00 	lea    0x0(%esi),%esi
     ef9:	8d bc 27 00 00 00 00 	lea    0x0(%edi),%edi
...
printint(int fd, int xx, int base, int sgn)
{
     ...
     f03:	57                   	push   %edi
     f04:	56                   	push   %esi
     f05:	89 ce                	mov    %ecx,%esi
     f07:	53                   	push   %ebx
     f08:	83 ec 1c             	sub    $0x1c,%esp
  char buf[16];
  int i, neg;
  uint x;
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  neg = 0;
  if(sgn && xx < 0){
     f11:	85 c9                	test   %ecx,%ecx
     f13:	74 04                	je     f19 <printint+0x19>
     f15:	85 d2                	test   %edx,%edx
     f17:	78 54                	js     f6d <printint+0x6d>
    neg = 1;
    x = -xx;
  } else {
    x = xx;
     f19:	89 d0                	mov    %edx,%eax
     f1b:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
     f22:	31 db                	xor    %ebx,%ebx
     f24:	8d 7d e4             	lea    -0x1c(%ebp),%edi
  }

  i = 0;
  do{
    buf[i++] = digits[x % base];
     f27:	31 d2                	xor    %edx,%edx
     f29:	f7 f6                	div    %esi
     f2b:	89 c1                	mov    %eax,%ecx
     f2d:	0f b6 82 46 13 00 00 	movzbl 0x1346(%edx),%eax
     f34:	88 04 3b             	mov    %al,(%ebx,%edi,1)
     f37:	83 c3 01             	add    $0x1,%ebx
  }while((x /= base) != 0);
     f3a:	85 c9                	test   %ecx,%ecx
     f3c:	89 c8                	mov    %ecx,%eax
     f3e:	75 e7                	jne    f27 <printint+0x27>
  if(neg)
     f40:	8b 45 e0             	mov    -0x20(%ebp),%eax
     f43:	85 c0                	test   %eax,%eax
     f45:	74 08                	je     f4f <printint+0x4f>
    buf[i++] = '-';
     f47:	c6 44 1d e4 2d       	movb   $0x2d,-0x1c(%ebp,%ebx,1)
     f4c:	83 c3 01             	add    $0x1,%ebx
     f4f:	8d 1c 1f             	lea    (%edi,%ebx,1),%ebx

  while(--i >= 0)
    putc(fd, buf[i]);
     f52:	0f be 53 ff          	movsbl -0x1(%ebx),%edx
     f56:	83 eb 01             	sub    $0x1,%ebx
     f59:	8b 45 dc             	mov    -0x24(%ebp),%eax
     f5c:	e8 6f ff ff ff       	call   ed0 <putc>
    buf[i++] = digits[x % base];
  }while((x /= base) != 0);
  if(neg)
    buf[i++] = '-';
  ...
  while(--i >= 0)
     f61:	39 fb                	cmp    %edi,%ebx
     f63:	75 ed                	jne    f52 <printint+0x52>
    putc(fd, buf[i]);
}
     f65:	83 c4 1c             	add    $0x1c,%esp
     f68:	5b                   	pop    %ebx
     f69:	5e                   	pop    %esi
     f6a:	5f                   	pop    %edi
     f6b:	5d                   	pop    %ebp
  ...
  if(sgn && xx < 0){
    ...
    x = -xx;
     ...
     f6f:	f7 d8                	neg    %eax
     f71:	c7 45 e0 01 00 00 00 	movl   $0x1,-0x20(%ebp)
     f78:	eb a8                	jmp    f22 <printint+0x22>
...
printf(int fd, char *fmt, ...)
{
  ...
  for(i = 0; fmt[i]; i++){
     ...
     f8c:	0f b6 02             	movzbl (%edx),%eax
     f8f:	84 c0                	test   %al,%al
     f91:	0f 84 87 00 00 00    	je     101e <printf+0x9e>
  char *s;
  int c, i, state;
  uint *ap;
  ...
  state = 0;
  ap = (uint*)(void*)&fmt + 1;
     f97:	8d 4d 10             	lea    0x10(%ebp),%ecx
     f9a:	31 ff                	xor    %edi,%edi
     f9c:	31 f6                	xor    %esi,%esi
     f9e:	89 4d f0             	mov    %ecx,-0x10(%ebp)
     fa1:	eb 18                	jmp    fbb <printf+0x3b>
  for(i = 0; fmt[i]; i++){
    c = fmt[i] & 0xff;
    if(state == 0){
      if(c == '%'){
     fa3:	83 fb 25             	cmp    $0x25,%ebx
     fa6:	0f 85 7a 00 00 00    	jne    1026 <printf+0xa6>
     fac:	66 be 25 00          	mov    $0x25,%si
  int c, i, state;
  uint *ap;
  ...
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
     fb0:	83 c7 01             	add    $0x1,%edi
     fb3:	0f b6 04 3a          	movzbl (%edx,%edi,1),%eax
     fb7:	84 c0                	test   %al,%al
     fb9:	74 63                	je     101e <printf+0x9e>
    c = fmt[i] & 0xff;
    if(state == 0){
     fbb:	85 f6                	test   %esi,%esi
  uint *ap;
  ...
  for(i = 0; fmt[i]; i++){
    c = fmt[i] & 0xff;
     fbd:	0f b6 d8             	movzbl %al,%ebx
    if(state == 0){
     fc0:	74 e1                	je     fa3 <printf+0x23>
      if(c == '%'){
        state = '%';
      } else {
        putc(fd, c);
     ...
     fc2:	83 fe 25             	cmp    $0x25,%esi
     fc5:	75 e9                	jne    fb0 <printf+0x30>
      if(c == 'd'){
     fc7:	83 fb 64             	cmp    $0x64,%ebx
     fca:	0f 84 f0 00 00 00    	je     10c0 <printf+0x140>
        printint(fd, *ap, 10, 1);
        ap++;
      } else if(c == 'x' || c == 'p'){
     fd0:	83 fb 78             	cmp    $0x78,%ebx
     fd3:	74 64                	je     1039 <printf+0xb9>
     fd5:	83 fb 70             	cmp    $0x70,%ebx
     fd8:	74 5f                	je     1039 <printf+0xb9>
        printint(fd, *ap, 16, 0);
        ap++;
      } else if(c == 's'){
     fda:	83 fb 73             	cmp    $0x73,%ebx
     fdd:	8d 76 00             	lea    0x0(%esi),%esi
     fe0:	74 7e                	je     1060 <printf+0xe0>
          s = "(null)";
        while(*s != 0){
          putc(fd, *s);
          s++;
     ...
     fe2:	83 fb 63             	cmp    $0x63,%ebx
     fe5:	0f 84 b9 00 00 00    	je     10a4 <printf+0x124>
        putc(fd, *ap);
        ap++;
      } else if(c == '%'){
     feb:	83 fb 25             	cmp    $0x25,%ebx
     fee:	66 90                	xchg   %ax,%ax
     ff0:	0f 84 f2 00 00 00    	je     10e8 <printf+0x168>
        putc(fd, c);
      } else {
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
     ff6:	8b 45 08             	mov    0x8(%ebp),%eax
     ff9:	ba 25 00 00 00       	mov    $0x25,%edx
  int c, i, state;
  uint *ap;
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1001:	31 f6                	xor    %esi,%esi
        ...
        // Unknown % sequence.  Print it to draw attention.
        putc(fd, '%');
    1003:	e8 c8 fe ff ff       	call   ed0 <putc>
        putc(fd, c);
    1008:	8b 45 08             	mov    0x8(%ebp),%eax
    100b:	0f be d3             	movsbl %bl,%edx
    100e:	e8 bd fe ff ff       	call   ed0 <putc>
    1013:	8b 55 0c             	mov    0xc(%ebp),%edx
  int c, i, state;
  uint *ap;
  ...
  ap = (uint*)(void*)&fmt + 1;
  for(i = 0; fmt[i]; i++){
    1016:	0f b6 04 3a          	movzbl (%edx,%edi,1),%eax
    101a:	84 c0                	test   %al,%al
    101c:	75 9d                	jne    fbb <printf+0x3b>
        putc(fd, c);
      }
      state = 0;
    }
    ...
    1029:	0f be d3             	movsbl %bl,%edx
    102c:	e8 9f fe ff ff       	call   ed0 <putc>
    1031:	8b 55 0c             	mov    0xc(%ebp),%edx
    1034:	e9 77 ff ff ff       	jmp    fb0 <printf+0x30>
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
        ap++;
    ...
    103c:	b9 10 00 00 00       	mov    $0x10,%ecx
        ap++;
    1041:	31 f6                	xor    %esi,%esi
    } else if(state == '%'){
      if(c == 'd'){
        printint(fd, *ap, 10, 1);
        ap++;
        ...
        printint(fd, *ap, 16, 0);
    1043:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
    104a:	8b 10                	mov    (%eax),%edx
    104c:	8b 45 08             	mov    0x8(%ebp),%eax
    104f:	e8 ac fe ff ff       	call   f00 <printint>
    1054:	8b 55 0c             	mov    0xc(%ebp),%edx
        ap++;
    1057:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
    105b:	e9 50 ff ff ff       	jmp    fb0 <printf+0x30>
      } else if(c == 's'){
        s = (char*)*ap;
    1060:	8b 4d f0             	mov    -0x10(%ebp),%ecx
    1063:	8b 01                	mov    (%ecx),%eax
        ap++;
    1065:	83 c1 04             	add    $0x4,%ecx
    1068:	89 4d f0             	mov    %ecx,-0x10(%ebp)
        if(s == 0)
    106b:	b9 3f 13 00 00       	mov    $0x133f,%ecx
    1070:	85 c0                	test   %eax,%eax
    1072:	75 2c                	jne    10a0 <printf+0x120>
          s = "(null)";
        while(*s != 0){
    1074:	0f b6 01             	movzbl (%ecx),%eax
    1077:	84 c0                	test   %al,%al
    1079:	74 1e                	je     1099 <printf+0x119>
    107b:	89 cb                	mov    %ecx,%ebx
    107d:	8d 76 00             	lea    0x0(%esi),%esi
          putc(fd, *s);
    1080:	0f be d0             	movsbl %al,%edx
    1083:	8b 45 08             	mov    0x8(%ebp),%eax
    1086:	e8 45 fe ff ff       	call   ed0 <putc>
      } else if(c == 's'){
        s = (char*)*ap;
        ap++;
        if(s == 0)
          s = "(null)";
        while(*s != 0){
    108b:	0f b6 43 01          	movzbl 0x1(%ebx),%eax
    108f:	83 c3 01             	add    $0x1,%ebx
    1092:	84 c0                	test   %al,%al
    1094:	75 ea                	jne    1080 <printf+0x100>
    ...
    109b:	e9 10 ff ff ff       	jmp    fb0 <printf+0x30>
        ap++;
      } else if(c == 'x' || c == 'p'){
        printint(fd, *ap, 16, 0);
        ap++;
        ...
        s = (char*)*ap;
    10a0:	89 c1                	mov    %eax,%ecx
    10a2:	eb d0                	jmp    1074 <printf+0xf4>
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
        ...
        putc(fd, *ap);
    10a4:	8b 45 f0             	mov    -0x10(%ebp),%eax
        ap++;
    10a7:	31 f6                	xor    %esi,%esi
        while(*s != 0){
          putc(fd, *s);
          s++;
        }
        ...
        putc(fd, *ap);
    10a9:	0f be 10             	movsbl (%eax),%edx
    10ac:	8b 45 08             	mov    0x8(%ebp),%eax
    10af:	e8 1c fe ff ff       	call   ed0 <putc>
    10b4:	8b 55 0c             	mov    0xc(%ebp),%edx
        ap++;
    10b7:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
    10bb:	e9 f0 fe ff ff       	jmp    fb0 <printf+0x30>
      } else {
        putc(fd, c);
      }
    } else if(state == '%'){
    ...
    10d7:	e8 24 fe ff ff       	call   f00 <printint>
    10dc:	8b 55 0c             	mov    0xc(%ebp),%edx
        ap++;
    10df:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
    10e3:	e9 c8 fe ff ff       	jmp    fb0 <printf+0x30>
        }
      } else if(c == 'c'){
        putc(fd, *ap);
        ap++;
    ...
    10eb:	ba 25 00 00 00       	mov    $0x25,%edx
    10f0:	31 f6                	xor    %esi,%esi
    10f2:	e8 d9 fd ff ff       	call   ed0 <putc>
    10f7:	8b 55 0c             	mov    0xc(%ebp),%edx
    10fa:	e9 b1 fe ff ff       	jmp    fb0 <printf+0x30>
    10ff:	90                   	nop    
...
morecore(uint nu)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    11f5:	e8 b6 fc ff ff       	call   eb0 <sbrk>
  if(p == (char*) -1)
