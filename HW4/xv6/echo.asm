...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3ad:	0f b6 82 e5 06 00 00 	movzbl 0x6e5(%edx),%eax
...
printf(int fd, char *fmt, ...)
{
 ...
 4e8:	89 4d f0             	mov    %ecx,-0x10(%ebp)
        if(s == 0)
 4eb:	b9 de 06 00 00       	mov    $0x6de,%ecx
...
free(void *ap)
{
 ...
 581:	8b 0d 00 07 00 00    	mov    0x700,%ecx
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 5cc:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 5cd:	89 0d 00 07 00 00    	mov    %ecx,0x700
}
 5d3:	c3                   	ret    
  bp = (Header*) ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
 ...
 5f4:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 5f5:	89 0d 00 07 00 00    	mov    %ecx,0x700
}
 5fb:	c3                   	ret    
...
malloc(uint nbytes)
{
 ...
 609:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 60c:	8b 15 00 07 00 00    	mov    0x700,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
malloc(uint nbytes)
{
 ...
 62c:	8d 3c dd 00 00 00 00 	lea    0x0(,%ebx,8),%edi
    ...
    if(p == freep)
 633:	3b 0d 00 07 00 00    	cmp    0x700,%ecx
 ...
 652:	89 59 04             	mov    %ebx,0x4(%ecx)
      ...
      freep = prevp;
 655:	89 15 00 07 00 00    	mov    %edx,0x700
      return (void*) (p + 1);
...
morecore(uint nu)
{
 ...
 688:	e8 f3 fe ff ff       	call   580 <free>
  return freep;
 68d:	8b 15 00 07 00 00    	mov    0x700,%edx
      }
      freep = prevp;
      return (void*) (p + 1);
    }
...
morecore(uint nu)
{
 ...
 6ab:	eb a8                	jmp    655 <malloc+0x55>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
 6ad:	ba f8 06 00 00       	mov    $0x6f8,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 6b2:	c7 05 00 07 00 00 f8 	movl   $0x6f8,0x700
 6b9:	06 00 00 
 6bc:	c7 05 f8 06 00 00 f8 	movl   $0x6f8,0x6f8
 6c3:	06 00 00 
    base.s.size = 0;
 6c6:	c7 05 fc 06 00 00 00 	movl   $0x0,0x6fc
 6cd:	00 00 00 
