...
_hw4-test-err:     file format elf32-i386
...
void lengthy(int *value, int N, int k)
{
   ...
   6:	83 ec 10             	sub    $0x10,%esp
	...
	int i;
	level = level + 1;
   9:	8b 15 b0 08 00 00    	mov    0x8b0,%edx
  printf(stderr, "%s\n", msg);
  exit();
}
...
void lengthy(int *value, int N, int k)
{
  ...
  1b:	89 15 b0 08 00 00    	mov    %edx,0x8b0
	value[k] = level;
...
void lengthy(int *value, int N, int k)
{
  ...
  56:	8b 45 f0             	mov    -0x10(%ebp),%eax
	...
	level = level-1; 
  59:	83 2d b0 08 00 00 01 	subl   $0x1,0x8b0
	value[k] = 0;
...
inline void handle_error(const char *msg) {
  ...
  79:	c7 44 24 04 38 08 00 	movl   $0x838,0x4(%esp)
  80:	00 
  81:	89 44 24 08          	mov    %eax,0x8(%esp)
  85:	a1 ac 08 00 00       	mov    0x8ac,%eax
  8a:	89 04 24             	mov    %eax,(%esp)
  8d:	e8 ce 04 00 00       	call   560 <printf>
  exit();
  92:	e8 51 03 00 00       	call   3e8 <exit>
...
int main(int argc, const char *argv[])
{
  ...
  ad:	51                   	push   %ecx
  ae:	83 ec 14             	sub    $0x14,%esp
	if (nice(2*MAX_PRIO) != -1)
  b1:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
  b8:	e8 cb 03 00 00       	call   488 <nice>
  bd:	83 c0 01             	add    $0x1,%eax
  c0:	0f 85 d9 00 00 00    	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (nice(-2*MAX_PRIO) != -1)
  c6:	c7 04 24 f6 ff ff ff 	movl   $0xfffffff6,(%esp)
  cd:	e8 b6 03 00 00       	call   488 <nice>
  d2:	83 c0 01             	add    $0x1,%eax
  d5:	0f 85 c4 00 00 00    	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (setpriority(-1,0) != -1)
  db:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
  e2:	00 
  e3:	c7 04 24 ff ff ff ff 	movl   $0xffffffff,(%esp)
  ea:	e8 a9 03 00 00       	call   498 <setpriority>
  ef:	83 c0 01             	add    $0x1,%eax
  f2:	0f 85 a7 00 00 00    	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (getpriority(-1) != -1)
  f8:	c7 04 24 ff ff ff ff 	movl   $0xffffffff,(%esp)
  ff:	e8 8c 03 00 00       	call   490 <getpriority>
 104:	83 c0 01             	add    $0x1,%eax
 107:	0f 85 92 00 00 00    	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (setpriority(getpid(),2*MAX_PRIO) != -1)
 10d:	e8 56 03 00 00       	call   468 <getpid>
 112:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
 119:	00 
 11a:	89 04 24             	mov    %eax,(%esp)
 11d:	e8 76 03 00 00       	call   498 <setpriority>
 122:	83 c0 01             	add    $0x1,%eax
 125:	75 78                	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (setpriority(getpid(),-1) != -1)
 127:	e8 3c 03 00 00       	call   468 <getpid>
 12c:	c7 44 24 04 ff ff ff 	movl   $0xffffffff,0x4(%esp)
 133:	ff 
 134:	89 04 24             	mov    %eax,(%esp)
 137:	e8 5c 03 00 00       	call   498 <setpriority>
 13c:	83 c0 01             	add    $0x1,%eax
 13f:	75 5e                	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	
	if (setaffinity(-1, 0) != -1)
 141:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
 148:	00 
 149:	c7 04 24 ff ff ff ff 	movl   $0xffffffff,(%esp)
 150:	e8 53 03 00 00       	call   4a8 <setaffinity>
 155:	83 c0 01             	add    $0x1,%eax
 158:	75 45                	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (setaffinity(getpid(), -1) != -1)
 15a:	e8 09 03 00 00       	call   468 <getpid>
 15f:	c7 44 24 04 ff ff ff 	movl   $0xffffffff,0x4(%esp)
 166:	ff 
 167:	89 04 24             	mov    %eax,(%esp)
 16a:	e8 39 03 00 00       	call   4a8 <setaffinity>
 16f:	83 c0 01             	add    $0x1,%eax
 172:	75 2b                	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (setaffinity(getpid(), 3) != -1)
 174:	e8 ef 02 00 00       	call   468 <getpid>
 179:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
 180:	00 
 181:	89 04 24             	mov    %eax,(%esp)
 184:	e8 1f 03 00 00       	call   4a8 <setaffinity>
 189:	83 c0 01             	add    $0x1,%eax
 18c:	75 11                	jne    19f <main+0xff>
		handle_error("Error: error checking for syscalls should be implemented.\n");
	if (getaffinity(-1) != -1)
 18e:	c7 04 24 ff ff ff ff 	movl   $0xffffffff,(%esp)
 195:	e8 06 03 00 00       	call   4a0 <getaffinity>
 19a:	83 c0 01             	add    $0x1,%eax
 19d:	74 22                	je     1c1 <main+0x121>
int stdout = 1;
int stderr = 2;
inline void handle_error(const char *msg) {
  printf(stderr, "%s\n", msg);
 19f:	a1 ac 08 00 00       	mov    0x8ac,%eax
 1a4:	c7 44 24 08 54 08 00 	movl   $0x854,0x8(%esp)
 1ab:	00 
 1ac:	c7 44 24 04 38 08 00 	movl   $0x838,0x4(%esp)
 1b3:	00 
 1b4:	89 04 24             	mov    %eax,(%esp)
 1b7:	e8 a4 03 00 00       	call   560 <printf>
  exit();
 1bc:	e8 27 02 00 00       	call   3e8 <exit>
		handle_error("Error: error checking for syscalls should be implemented.\n");

	printf(stdout, "hw4-test-err succeeded\n");
 1c1:	a1 a8 08 00 00       	mov    0x8a8,%eax
 1c6:	c7 44 24 04 3c 08 00 	movl   $0x83c,0x4(%esp)
 1cd:	00 
 1ce:	89 04 24             	mov    %eax,(%esp)
 1d1:	e8 8a 03 00 00       	call   560 <printf>

	exit();
 1d6:	e8 0d 02 00 00       	call   3e8 <exit>
 1db:	90                   	nop    
 1dc:	90                   	nop    
 1dd:	90                   	nop    
 1de:	90                   	nop    
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 50d:	0f b6 82 97 08 00 00 	movzbl 0x897(%edx),%eax
...
printf(int fd, char *fmt, ...)
{
 ...
 648:	89 4d f0             	mov    %ecx,-0x10(%ebp)
        if(s == 0)
 64b:	b9 90 08 00 00       	mov    $0x890,%ecx
...
free(void *ap)
{
 ...
 6e1:	8b 0d bc 08 00 00    	mov    0x8bc,%ecx
static Header base;
static Header *freep;
...
free(void *ap)
{
 ...
 72c:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 72d:	89 0d bc 08 00 00    	mov    %ecx,0x8bc
}
 733:	c3                   	ret    
  bp = (Header*) ap - 1;
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
 ...
 754:	5d                   	pop    %ebp
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
 755:	89 0d bc 08 00 00    	mov    %ecx,0x8bc
}
 75b:	c3                   	ret    
...
malloc(uint nbytes)
{
 ...
 769:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 76c:	8b 15 bc 08 00 00    	mov    0x8bc,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
...
malloc(uint nbytes)
{
 ...
 78c:	8d 3c dd 00 00 00 00 	lea    0x0(,%ebx,8),%edi
    ...
    if(p == freep)
 793:	3b 0d bc 08 00 00    	cmp    0x8bc,%ecx
 ...
 7b2:	89 59 04             	mov    %ebx,0x4(%ecx)
      ...
      freep = prevp;
 7b5:	89 15 bc 08 00 00    	mov    %edx,0x8bc
      return (void*) (p + 1);
...
morecore(uint nu)
{
 ...
 7e8:	e8 f3 fe ff ff       	call   6e0 <free>
  return freep;
 7ed:	8b 15 bc 08 00 00    	mov    0x8bc,%edx
      }
      freep = prevp;
      return (void*) (p + 1);
    }
...
morecore(uint nu)
{
 ...
 80b:	eb a8                	jmp    7b5 <malloc+0x55>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    base.s.size = 0;
 80d:	ba b4 08 00 00       	mov    $0x8b4,%edx
  Header *p, *prevp;
  uint nunits;
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 812:	c7 05 bc 08 00 00 b4 	movl   $0x8b4,0x8bc
 819:	08 00 00 
 81c:	c7 05 b4 08 00 00 b4 	movl   $0x8b4,0x8b4
 823:	08 00 00 
    base.s.size = 0;
 826:	c7 05 b8 08 00 00 00 	movl   $0x0,0x8b8
 82d:	00 00 00 
