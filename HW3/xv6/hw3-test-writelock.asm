...
_hw3-test-writelock:     file format elf32-i386
...
int main() {
 ...
 132:	0f 88 ad 00 00 00    	js     1e5 <main+0x135>
    stacks[i] = malloc(STACK_SIZE);
 ...
 155:	0f 88 94 00 00 00    	js     1ef <main+0x13f>
    children[i] = tfork(child_main, &m, stacks[i] + STACK_SIZE);
    if (children[i] < 0)
      handle_error("error on tfork()");
  }
...
inline int time_equal(int a, int b) {
 ...
 19f:	78 19                	js     1ba <main+0x10a>
    return 0;
  if (a > b) {
 1a1:	3d e8 03 00 00       	cmp    $0x3e8,%eax
 1a6:	ba e8 03 00 00       	mov    $0x3e8,%edx
 1ab:	7f 2f                	jg     1dc <main+0x12c>
 1ad:	89 d1                	mov    %edx,%ecx
 1af:	29 c1                	sub    %eax,%ecx
 1b1:	8d 04 89             	lea    (%ecx,%ecx,4),%eax
 1b4:	01 c0                	add    %eax,%eax
 1b6:	39 c2                	cmp    %eax,%edx
 1b8:	7d 3f                	jge    1f9 <main+0x149>
  }
  return 10 * (b - a) <= b;
}
...
inline void handle_error(const char *msg) {
  printf(stderr, "%s\n", msg);
 1ba:	c7 44 24 08 00 09 00 	movl   $0x900,0x8(%esp)
 1c1:	00 
  exit();
 1c2:	a1 40 09 00 00       	mov    0x940,%eax
 1c7:	c7 44 24 04 98 08 00 	movl   $0x898,0x4(%esp)
 1ce:	00 
 1cf:	89 04 24             	mov    %eax,(%esp)
 1d2:	e8 e9 03 00 00       	call   5c0 <printf>
 1d7:	e8 6c 02 00 00       	call   448 <exit>
...
inline int time_equal(int a, int b) {
 ...
 1de:	b8 e8 03 00 00       	mov    $0x3e8,%eax
 1e3:	eb c8                	jmp    1ad <main+0xfd>
  }
  return 10 * (b - a) <= b;
}
...
inline void handle_error(const char *msg) {
  printf(stderr, "%s\n", msg);
 1e5:	c7 44 24 08 9c 08 00 	movl   $0x89c,0x8(%esp)
 1ec:	00 
 1ed:	eb d3                	jmp    1c2 <main+0x112>
 1ef:	c7 44 24 08 ad 08 00 	movl   $0x8ad,0x8(%esp)
 1f6:	00 
 1f7:	eb c9                	jmp    1c2 <main+0x112>
    handle_error("Did not finish in a correct time.");

  for (i = 0; i < N_THREADS; i++)
    free(stacks[i]);
 1f9:	8b 45 a0             	mov    -0x60(%ebp),%eax
 1fc:	bb 02 00 00 00       	mov    $0x2,%ebx
 201:	89 04 24             	mov    %eax,(%esp)
 204:	e8 37 05 00 00       	call   740 <free>
 209:	8b 44 9d 9c          	mov    -0x64(%ebp,%ebx,4),%eax
 20d:	83 c3 01             	add    $0x1,%ebx
 210:	89 04 24             	mov    %eax,(%esp)
 213:	e8 28 05 00 00       	call   740 <free>
  ...
  for (i = 0; i < N_THREADS; i++)
 218:	83 fb 0b             	cmp    $0xb,%ebx
 21b:	75 ec                	jne    209 <main+0x159>
    free(stacks[i]);

  printf(stdout, "hw3-test-writelock succeeded\n");
 21d:	a1 3c 09 00 00       	mov    0x93c,%eax
 222:	c7 44 24 04 be 08 00 	movl   $0x8be,0x4(%esp)
 229:	00 
 22a:	89 04 24             	mov    %eax,(%esp)
 22d:	e8 8e 03 00 00       	call   5c0 <printf>
  
  exit();
 232:	e8 11 02 00 00       	call   448 <exit>
 237:	90                   	nop    
 238:	90                   	nop    
 239:	90                   	nop    
 23a:	90                   	nop    
